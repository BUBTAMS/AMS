...
def timetable(request, class_id):
    ...
    matrix = [['' for i in range(14)] for j in range(6)]
...
def t_timetable(request, teacher_id):
    ...
    class_matrix = [[True for i in range(14)] for j in range(6)]
    for i, d in enumerate(DAYS_OF_WEEK):
        t = 0
        for j in range(14):
            if j == 0:
...
def marks_list(request, stud_id):
    ...
    for ass in ass_list:
        try:
            ...
            sc.marks_set.create(type='I', name='Class test 3')
            sc.marks_set.create(type='E', name='Quiz Test')
            sc.marks_set.create(type='E', name='Mid Exam')
